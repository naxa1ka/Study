...
const int N_MAX = 10; //максимальное число строк в матрице
const int M_MAX = 10; //максимальное число столбцов в матрице
const char* FILENAME_1 = "tests_1/src6.txt"; //путь к исходным файлам для 1 матрицы
...
void PrintArray(int ARRAY[][M_MAX], int N, int M); //печать массива
void MaxArray(int ARRAY[][M_MAX], int N, int M);  //поиск максимума среди строк в которых есть хотя бы один положительный элемент
void SwapArray(int ARRAY[][M_MAX], int N, int M); //замена строк четных и нечетных номеров
void GetCode(int code);

void CHECK_ERRORS(int ARRAY[][M_MAX], int& N, int& M, int& CodeError, const char* FILENAME); //проверка на ошибки и ввод
...
int main() {
	...
	int ARRAY_1[N_MAX][M_MAX]; //объявление первой матрицы
	int ARRAY_2[N_MAX][M_MAX]; //объявление второй матрицы

	CHECK_ERRORS(ARRAY_1, N, M, CodeError, FILENAME_1);
	...
	CodeError = 0;
	CHECK_ERRORS(ARRAY_2, N, M, CodeError, FILENAME_2);
	if (CodeError == 0) {
		cout << "Исходные данные второго массива: ";
...
void MaxArray(int ARRAY[][M_MAX], int N, int M) {
	cout << endl;
...
void SwapArray(int ARRAY[][M_MAX], int N, int M) {
	int buff;
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < M / 2; j++) {
			buff = ARRAY[i][j];
...
void CHECK_ERRORS(int ARRAY[][M_MAX], int& N, int& M, int& CodeError, const char* FILENAME) {
